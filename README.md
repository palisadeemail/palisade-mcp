...
Every tool that reads or changes Palisade's own records is scoped to the organization your API key belongs to; `audit_domain` and `validate_spf_include` are the exceptions, taking a domain name rather than an id and reading public DNS for anything.
